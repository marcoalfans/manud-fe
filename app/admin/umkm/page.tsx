...
const initialData: UmkmItem[] = [
  {
    id: 'u1',
    name: 'Batik Sari',
    category: 'Fashion',
    location: 'Pusat Kota',
    photoUrl: '/batik-sari.jpg',
  ...
  },
  {
    ...
    name: 'Kopi Lestari',
    category: 'Kopi',
    location: 'Lereng Barat',
    photoUrl: '/kopi-lestari.jpg',
...
export default function UmkmPage() {
  const [items, setItems] = useState<UmkmItem[]>(initialData)
  ...
  function addItem(values: FormValues) {
    const id = `u_${Date.now().toString(36)}_${Math.round(Math.random() * 1e4)}`
    setItems(prev => [
      {
        id,
        name: values.name,
  ...
  return (
    <div className='flex flex-col gap-6'>
      <header className='flex items-center justify-between'>
        <div>
          <h1 className='text-2xl font-semibold'>UMKM</h1>
          <p className='text-muted-foreground text-sm'>Manage SME entries</p>
        </div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className='gap-2'>
          <Plus className='size-4' /> Add UMKM
        </Button>
      </DialogTrigger>
      <DialogContent className='sm:max-w-md'>
        <DialogHeader>
          <DialogTitle>Add UMKM</DialogTitle>
          <DialogDescription>
            Fill out the form to add a new SME entry.
          </DialogDescription>
        </DialogHeader>
